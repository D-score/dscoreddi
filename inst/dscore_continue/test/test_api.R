## Test script for plumber API
## This is an adapted version of global.R
...
library(gridExtra)
library(cowplot)
# library(dscoreddi) --> API
# library(dinstrument) ---> API
...
# Use plumber default port on localhost (must be running!)
resp <- james_get("http://127.0.0.1:3387/reference?population=dutch")
head(resp$parsed, 2)

# same, but build request from parts
host <- "http://127.0.0.1:3387"
resp <- james_get(host, path = "reference", query = "population=dutch")

# we've got our first asset
...
itembank <- itembank %>% filter(key == "dutch") %>%
...
resp <- james_get(host = host, path = "itemtableVWO")
itemtableVWO <- resp$parsed
