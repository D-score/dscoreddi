...
reference1 <- dscore::builtin_references %>% filter(population == "dutch" & key == "gsed2406") %>% 
...
example <- itembank_vwc[36,c("item", "tau", "label")]
p1 <- 1:100

#input reference
reference <- dscoreddi::expanded_reference %>% mutate(agemos = age * 12)
...
```{r}
vwcdf <- dmetric::calculate_age_equivalents(itembank = itembank_vwc[36,],scalefactor = 2.099986, p = c(10, 50, 90), reference = expanded_reference)
